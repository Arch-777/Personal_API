...
def embed_item(self, item_id: str, user_id: str, chunk_count: int | None = None) -> dict[str, Any]:
    try:
        ...
        raise NonRetryableSyncError("Invalid item_id or user_id passed to embedding task") from exc
    ...
    try:
        with SessionLocal() as db:
            ...
            if item is None:
                raise NonRetryableSyncError("Item not found for embedding")
        ...
        logger.exception("Embedding worker failed for item_id=%s user_id=%s", item_id, user_id)
        raise
